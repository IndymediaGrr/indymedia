...
\***************************************************************************/
// Surcharge pour indymedia - ne jamais mettre l'email de session auteur
// force a rentrer un pseudo ou rien
...
function inc_forum_insert_dist($objet, $id_objet, $id_forum, $force_statut = NULL) {
	...
	$c['auteur'] = sinon($GLOBALS['visiteur_session']['session_nom'],
		' ');
	$c['email_auteur'] = sinon($GLOBALS['visiteur_session']['session_email'],
		' ');
	...
	// ne pas garder les pseudos
	unset($GLOBALS['visiteur_session']['session_nom']);
	unset($GLOBALS['visiteur_session']['session_email']);
