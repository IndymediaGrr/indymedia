...
function minipres($titre='', $corps="", $onload='', $all_inline = false)
{
	...
	if (!$titre) {
		...
		$corps = generer_form_ecrire('accueil', '','',
						isset($GLOBALS['visiteur_session']['statut'])?_T('public:accueil_site'):_T('public:lien_connecter')
		);
		// spip_log($GLOBALS['visiteur_session']['nom'] . " $titre " . $_SERVER['REQUEST_URI']);
	}
	// spip_log("corps : ----- ".$corps."   ----- SPAM #COMPLET",	'indy'._LOG_AVERTISSEMENT);	

	// if (! isset($GLOBALS['visiteur_session']['statut'])){	
			// $fin = strpos($corps, "div>Attention");
			// $corps = substr($corps,0,$fin);	
		if(strpos($corps,"é") or strpos($corps,"è"))
			spip_log("SPAM FR #COMPLET? : ----- ".couper($corps, 1000, '  (ETC, ...)')."   ----- SPAM FR? #COMPLET",	'indy'._LOG_AVERTISSEMENT);	
		else if(strpos($corps,"a>"))
			spip_log("SPAM lien a> ? : ----- ".couper($corps, 100, '  (ETC, ...)')."  ---  coupe100? #LINK<a>",	'indy'._LOG_AVERTISSEMENT);		
		else
			spip_log("SPAM ? : ---- ".couper($corps, 250, '  (ETC, ...)')."  --- SPAM coupe250 #AUTRE ",	'indy'._LOG_AVERTISSEMENT);
	// }
	if (!_AJAX){
