 ...
 */
function formulaires_publication_ouverte_elsewhere_charger_dist()
{
	$values = array();
	...
	$values['id_rubrique'] 	= lire_config('indymedia/rubrique_elsewhere')?lire_config('indymedia/rubrique_elsewhere'):0;		// rubrique de la contrib
	...
	$values['url_site'] = _request('url_site');
	/*
	...
	else if ($values['id_article'] &&  est_admin())
	{
		...
		$row = sql_fetsel(
			array('texte','extra','id_rubrique','titre','date','url_site'),
			array('spip_articles'),
 ...
 */
function formulaires_publication_ouverte_elsewhere_verifier_dist()
{
	$erreurs = array();
	$values = array();	
	$values['titre'] 		= _request('titre');			// titre de la contrib
	...
	if (!$values['url_site']) 
		$erreurs['url_site'] = _T('publication_erreur_url_site');
 ...
 */
function formulaires_publication_ouverte_elsewhere_traiter_dist()
{
	$message = array();
	$values = array();
	$values['etape'] 		= _request('etape');			// etape actuelle
